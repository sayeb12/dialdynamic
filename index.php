...
<head>
    ...
    <style>
        ...
        /* New Year Popup Styles - Communication Theme */
        .new-year-popup {
            position: fixed;
            top: 0;
            left: 0;
        ...
        .new-year-popup.show .ny-popup-content {
            transform: scale(1);
        }
        ...
        .ny-year-number {
            ...
            background: linear-gradient(45deg, #0ef, #00b7ff);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            background-clip: text;
        ...
        /* Communication Particle Effects */
        .communication-particle {
            ...
            pointer-events: none;
            z-index: 99998;
            animation: communicationFloat 3s linear forwards;
        ...
        .signal-particle {
            ...
            width: 2px;
            height: 40px;
            background: linear-gradient(to top, transparent, #0ef);
            pointer-events: none;
            z-index: 99998;
            animation: signalTransmit 2s linear infinite;
        }

        @keyframes communicationFloat {
            0% {
...
<body class="<?php echo $justConnected ? 'connected-state' : ''; ?>">
    ...
    <?php else: ?>
        ...
        <div id="website-content">
            ...
            <!-- Happy New Year 2026 Popup -->
            <div id="newYearPopup" class="new-year-popup">
                <div class="ny-popup-content">
                    <div class="ny-close-btn" onclick="closeNewYearPopup()">
                        <i class="fas fa-times"></i>
                    </div>

                    <div class="ny-popup-header">
                        <h2>Happy New Year</h2>
                        <div class="ny-year-number">2026</div>
                    </div>

                    <div class="ny-communication-icons">
                        <div class="ny-service-icon" title="Voice Calls">
                            <div class="ny-icon-circle">
                                <i class="fas fa-phone-volume"></i>
                            </div>
                        </div>
                        <div class="ny-service-icon" title="Text Messages">
                            <div class="ny-icon-circle">
                                <i class="fas fa-comment-dots"></i>
                            </div>
                        </div>
                        <div class="ny-service-icon" title="Video Calls">
                            <div class="ny-icon-circle">
                                <i class="fas fa-video"></i>
                            </div>
                        </div>
                        <div class="ny-service-icon" title="Conference Calls">
                            <div class="ny-icon-circle">
                                <i class="fas fa-users"></i>
                            </div>
                        </div>
                        <div class="ny-service-icon" title="Cloud Communication">
                            <div class="ny-icon-circle">
                                <i class="fas fa-cloud"></i>
                            </div>
                        </div>
                        <div class="ny-service-icon" title="Secure Connection">
                            <div class="ny-icon-circle">
                                <i class="fas fa-shield-alt"></i>
                            </div>
                        </div>
                    ...
                    <div class="ny-popup-message">
                        <p>
                            Welcoming 2026 with enhanced connections and seamless communication!
                        </p>
                        <p>
                            May this year bring crystal clear conversations and uninterrupted connections.
                        </p>
                    </div>

                    <p class="ny-tagline">From the Dial Dynamic Team</p>
                </div>
            </div>
        </div>
    <?php endif; ?>
    ...
    <?php if (!$showCall): ?>
        <!-- New Year Popup JavaScript -->
        <script>
            // Function to create communication-themed particle effects
            function createCommunicationEffects() {
                const popup = document.getElementById('newYearPopup');
                if (!popup) return;

                const popupRect = document.querySelector('.ny-popup-content').getBoundingClientRect();
                ...
                for (let i = 0; i < 15; i++) {
                    setTimeout(() => {
                        ...
                        // Position particles around the popup
                        const angle = Math.random() * Math.PI * 2;
                        const distance = 120 + Math.random() * 80;
                        const x = popupRect.left + popupRect.width / 2 + Math.cos(angle) * distance;
                        const y = popupRect.top + popupRect.height / 2 + Math.sin(angle) * distance;
                ...
                // Create signal transmission effects
                for (let i = 0; i < 4; i++) {
                    setTimeout(() => {
                        const signal = document.createElement('div');
                        signal.className = 'signal-particle';

                        // Position signals around the icons
                        const iconRects = document.querySelectorAll('.ny-icon-circle');
                        if (iconRects.length > 0) {
                            const randomIcon = iconRects[Math.floor(Math.random() * iconRects.length)];
                            ...
                            signal.style.transformOrigin = 'bottom center';

                            // Random rotation for direction
                            const rotation = Math.random() * 360;
                            signal.style.transform = `rotate(${rotation}deg)`;

                            popup.appendChild(signal);
            ...
            // Function to show New Year popup
            ...
            // Function to close the New Year popup
            function closeNewYearPopup() {
                ...
                // Clean up particles
                setTimeout(() => {
                    const particles = popup.querySelectorAll('.communication-particle, .signal-particle');
            ...
            document.addEventListener('DOMContentLoaded', function() {
                // If there's a greeting overlay, wait for it to finish
                const greetingOverlay = document.getElementById('greeting-overlay');

                if (greetingOverlay) {
                    // Show New Year popup 4 seconds after page load (1 second after greeting overlay fades)
                    setTimeout(showNewYearPopup, 4000);
                } else {
                    // Show immediately (or after 2 seconds if you want)
                    setTimeout(showNewYearPopup, 2000);
                }
